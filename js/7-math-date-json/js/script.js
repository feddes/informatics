...
Товар из глобального массива positions, объект:
...
Количество товаров, которое нужно поставить, число.
...
Процесс выполнения
Создайте функцию lotCalculator, принимающую нужное количество аргументов.
Рассчитайте минимальное количество партий, необходимых для заказа, на основании количества товаров, которое нужно поставить и размера партии из свойств товара.
Рассчитайте общую стоимость товаров с учетом рассчитанного количество партий и стоимости товара.
Создайте объект со свойствами lots и total, поместив туда количество партий и общую стоимость товаров.
Вызовите функцию lotCalculator, используя данные из массива positions и придуманные самостоятельно.
Каждый раз выводите полученный результат в консоль в виде:
...
Поставщик, объект со свойствами:
name — название поставщика (строка),
deferPeriod — срок отсрочки в земных сутках (число);
Сумма отгрузки, число;
Дата отгрузки, от которой считать отсрочку, объект Date. Функция ничего не должна возвращать. Она должна рассчитать дату оплаты, используя дату отгрузки и свойство поставщика deferPeriod. И поместить в глобальный массив deferedPayments объект со свойствами:
...
Процесс выполнения
Объявите константу deferedPayments, присвойте в неё пустой массив.
Создайте функций deferPay, принимающую нужное количество аргументов.
Вычислите дату платежа, используя дату отгрузки и свойство поставщика deferPeriod.
Создайте новый объект со свойствами producer, amount и paymentDate, поместив туда поставщика, сумму платежа и рассчитанную дату платежа.
Поместите объект в массив deferedPayments.
Вызовите созданную функцию несколько раз, используя данные из массива positions и придуманные самостоятельно.
Выведите содержимое массива deferedPayments в формате:
...
to — буквенный код валюты, в которую необходимо перевести, строка. Функция должна вернуть число, округленное до сотых.
...
Получите, используя функцию loadCurrencyJSON, информацию о курсах в формате JSON.
Преобразуйте JSON в JavaScript-объект.
Получите курсы исходной и производной валют по их кодам из атрибутов from и to.
Осуществите расчет суммы в производной валюте.
Округлите полученное значение до сотых.
Верните полученное значение. Убедитесь, что примеры использования работают с вашей функцией и дают тот результат, который описан. И попробуйте свои варианты использования.
